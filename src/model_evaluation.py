...
def plot_model_results(model, pred_df, y_true_col, report_dir='./data/report'):
    """
    Plot and save crash probabilities and confusion matrix for a given model.
    """
    os.makedirs(report_dir, exist_ok=True)

    # Crash probabilities plot
    fig_crash_prob = px.histogram(pred_df, x=model, color=y_true_col, nbins=50, title=f'{model} Crash Probabilities')
    fig_crash_prob.update_layout(bargap=0.1)
    fig_crash_prob.write_image(f'{report_dir}/{model}_crash_probs.png')

    # Confusion matrix plot
    y_pred = np.where(pred_df[model] > 0.5, 1, 0)
    fig_cm = plot_confusion_matrix(pred_df[y_true_col], y_pred, title=model)
    fig_cm.write_image(f'{report_dir}/{model}_confusion_matrix.png')


def save_model_evaluation_reports(metrics_result, report_dir='./data/report', horizon_months=12):
    """
    Save evaluation metrics summary to CSV.
    """
    os.makedirs(report_dir, exist_ok=True)
    metrics_result.to_csv(f'{report_dir}/{horizon_months}M_metrics_summary.csv')

# Note: Interactive visualization widgets (e.g., ipywidgets) have been removed or commented out as per instructions.